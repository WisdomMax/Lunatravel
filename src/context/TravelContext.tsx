          ...
          if (s && Object.keys(s).length > 0) {
            ...
            setState(prev => {
              ...
              return {
                ...
                isBgmPlaying: s.is_bgm_playing !== undefined ? s.is_bgm_playing : (s.isBgmPlaying !== undefined ? s.isBgmPlaying : prev.isBgmPlaying),
                isInitialized: true
              };
            });
          }
...
- Your personality traits and the defined relationship are the MOST important elements of your character. Always stay in character.`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 서버에도 실시간 저장 (isInitialized 가드 추가로 초기 데이터 유실 방지)
      ...
      fetch('/api/settings', {
        ...
        body: JSON.stringify({ 
          ...
          is_bgm_playing: state.isBgmPlaying,
  ...
  }, [state.history, state.currentLocation, state.memory, state.persona, state.lunaName, state.lunaSelection, state.photos, state.bookmarks, state.chatHistories, state.photoHistories, state.lunaPhotos, state.nearbyPlaces, state.isBgmPlaying, state.bgmVolume]);
...
- Speak naturally and act as a real travel companion.${ageInstruction}`;
  ...
  const takeTravelPhoto = useCallback(async (location: Location, heading: number, pitch: number, zoom: number, customPrompt?: string) => {
    ...
    try {
      ...
      const newPhoto = {
        ...
        url: resultUrl,
        locationName: location.name || 'Travel',
        timestamp: Date.now()
      };
  ...
  const updateSettings = useCallback(async (updates: any) => {
    ...
    setState(prev => {
      ...
      const newState = {
        ...
        isBgmPlaying: updates.isBgmPlaying !== undefined ? updates.isBgmPlaying : (updates.is_bgm_playing !== undefined ? updates.is_bgm_playing : prev.isBgmPlaying)
      };
      ...
      try {
        await fetch('/api/settings', {
          ...
          body: JSON.stringify({
            ...
            is_bgm_playing: finalState.isBgmPlaying,
  ...
  return (
    <TravelContext.Provider value={{
      ...
      toggleBgm: () => setState(prev => {
        const nextPlaying = !prev.isBgmPlaying;
        updateSettings({ is_bgm_playing: nextPlaying });
        return { ...prev, isBgmPlaying: nextPlaying };
      }),
      setBgmVolume: (volume: number) => {
        setState(prev => ({ ...prev, bgmVolume: volume }));
        updateSettings({ bgm_volume: volume });
      },
